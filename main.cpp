...
struct Player {
	...
	void update() {
		...
		{
			if (CheckCollisionCircles(asters[i].pos, asters[i].size * 10.f, pos, SIZE * 5.f)) {
		...
		for (int i = 0; i < 3; i++) {
			tri[i] = {
				cos_theta * (tri[i].x - pos.x) - sin_theta * (tri[i].y - pos.y) + pos.x,
				sin_theta * (tri[i].x - pos.x) + cos_theta * (tri[i].y - pos.y) + pos.y
			};
		}
		DrawTriangle(
			tri[0],
		...
		vel = Vector2Add(
			vel,
			Vector2Scale(
				{cosf(angle - PI/2.f), sinf(angle - PI/2.f)},
		...
		if (cooldown <= 0.f) {
			if (IsKeyDown(KEY_SPACE)) {
				spawnBullet(pos, Vector2Scale({cosf(angle - PI/2.f), sinf(angle - PI/2.f)}, BULLET_SPEED));
				cooldown = COOLDOWN;
			}
		} else {
			cooldown -= dt;
	...
	while (!WindowShouldClose()) {
		BeginDrawing();
			...
			case GAME: {
				...
				DrawText(TextFormat("Speed: %.2f", Vector2Length(player.vel)), 0, 60, 30, WHITE);
